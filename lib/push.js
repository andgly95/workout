...
function addSub(d, raw) {
  ...
  d.push.subs = d.push.subs.filter(s => s.endpoint !== clean.endpoint);
  d.push.subs.push(clean);
  store.save();
  return clean;
}

function removeSub(d, endpoint) {
  const before = d.push.subs.length;
  d.push.subs = d.push.subs.filter(s => s.endpoint !== String(endpoint || ''));
  if (d.push.subs.length !== before) store.save();
  return before - d.push.subs.length;
}

// Fan out to every device. A 404/410 means the subscription is dead for good, so
// it gets dropped rather than retried forever.
// Sends to ONE person's devices. There is no route to anyone else's endpoints.
async function send(d, payload) {
  if (!d.push.subs.length) return 0;
  ...
  let sent = 0;
  for (const sub of [...d.push.subs]) {
    try {
      await webpush.sendNotification({ endpoint: sub.endpoint, keys: sub.keys }, body);
      sent++;
    } catch (e) {
      const code = e && e.statusCode;
      if (code === 404 || code === 410) removeSub(d, sub.endpoint);
...
// One pass over EVERYBODY. Each person's schedules are read against their own
// slice and their own devices; the loop is the only thing that spans users, and
// it never carries anything from one to the next.
async function tick(now = new Date()) {
  const today = calendar.dayStr(now);
  const nowMin = now.getHours() * 60 + now.getMinutes();
  const fired = [];
  for (const [uid, d] of Object.entries(state.data || {})) {
    const u = (state.users || {})[uid];
    // Nobody who has been denied, or is still waiting, gets nagged.
    if (u && u.status !== 'active') continue;
    fired.push(...await tickUser(d, today, nowMin));
  }
  return fired;
}

async function tickUser(d, today, nowMin) {
  if (!d.push.subs.length) return [];
  const fired = [];

  for (const plan of d.plans || []) {
    if (plan.archived) continue;
    const sent = d.push.sent[plan.id];
    if (!schedule.reminderDue(plan.schedule, today, lastDoneFor(d, plan.id), nowMin, sent)) continue;
    const tried = d.push.tried[plan.id];
    if (tried && tried.day === today && tried.n >= MAX_TRIES) continue;

    const names = plan.exerciseIds
      .map(id => (d.exercises.find(e => e.id === id) || {}).name)
      .filter(Boolean);
    const sentTo = await send(d, {
      title: 'Lift',
    ...
    if (!sentTo) {
      const t = d.push.tried[plan.id];
      d.push.tried[plan.id] = t && t.day === today ? { day: today, n: t.n + 1 } : { day: today, n: 1 };
      store.save();
      continue;
    }
    d.push.sent[plan.id] = today;
    delete d.push.tried[plan.id];
    store.save();
    fired.push(plan.id);
  }
  return fired;
...
module.exports = { publicKey, configured, addSub, removeSub, send, tick, tickUser, start, stop, lastDoneFor, CHECK_MS, MAX_TRIES };
