...
// A brand-new person's starting program: the six seeded machines in one plan.
// Same shape a legacy store migrates INTO, so there is one definition of "what a
// user's data looks like" rather than two that drift.
function seedUserData(mint) {
  const d = {
    exercises: [], plans: [], activePlanId: null, progress: {}, workouts: [],
...
// Migrate ONE user's data slice. Handles both a store written before plans
// existed (flat `progress[exerciseId]`, a global `settings.restSec`, and
// `includeOptional` deciding whether the leg curl was in the program) and a fresh
// empty slice, which is the same problem with nothing to carry over.
