...
/* ── two ways to prove who you are, one set of users ──────────────────────
   Cloudflare Access and Google Sign-In are identity SOURCES. Both end up in
   `upsertUser`, so the approval queue, the data slices and everything downstream
   are identical whichever way somebody arrived — and adding a third source later
   is a function, not a refactor. */

// Find or create the person behind a verified identity.
//
// Linking by email is safe here and only here: BOTH sources verify the address
// before we ever see it (Google via `email_verified`, Access because it is the
// thing it just authenticated). Without linking, signing in through Access and
// then directly through Google would silently hand you two accounts and two
// separate training logs.
function upsertUser(claims) {
  let u = state.users[claims.id];
  if (!u) {
    const linked = Object.values(state.users).find(x => x.email === claims.email);
    if (linked) {
      // Same person, second door. Record the alias so the id they arrive with
      // keeps resolving, and leave their data exactly where it is.
      state.users[claims.id] = linked;
      linked.alsoKnownAs = [...new Set([...(linked.alsoKnownAs || []), claims.id])];
      u = linked;
    }
  }
  if (!u) {
    ...
    u = {
      id: claims.id, email: claims.email, name: claims.name, picture: claims.picture,
      owner,
    ...
    u.name = u.name || claims.name;
    if (claims.picture) u.picture = claims.picture;
    u.lastSeen = Date.now();
  }
  store.save();
  return u;
...
module.exports = function mount(app) {
  ...
  //
  // Cloudflare Access goes first: if it is in front of this origin then the person
  // is ALREADY authenticated by the time the request lands, and asking them to
  // press a Google button as well would be theatre. Its assertion is verified, not
  // trusted — see lib/cfaccess.js for why that distinction is the whole file.
  app.use(async (req, res, next) => {
    let u = userFrom(req);
    if (!u && cfaccess.configured()) {
      try {
        const claims = await cfaccess.identify(req);
        if (claims) {
          u = upsertUser(claims);
          // Mint our own session too, so every later request is a cheap cookie
          // check rather than a JWT verification, and so the pending/approved
          // machinery is identical whichever door you came through.
          res.setHeader('Set-Cookie', auth.cookieHeader(auth.sign(u.id), { secure: req.secure }));
          if (u.status !== 'active') u = null;
        }
      } catch (e) { console.error('cfaccess:', e.message); }
    }
    if (u) {
      req.user = u;
      req.d = dataFor(u.id);
  ...
  app.post('/api/auth/google', async (req, res) => {
    ...
    // Same path Access takes — one place decides who is who, so the two doors
    // can never disagree about your status or your data.
    const u = upsertUser({ ...claims, id: `google:${claims.id}`, via: 'google' });
  ...
  // Setup helper: what is Cloudflare actually sending this origin?
  //
  // The AUD tag is buried in the Zero Trust dashboard and moves between releases,
  // but Access puts it in every assertion it signs — so the easiest place to read
  // it is here. Open this URL in a browser that has been through Access and the
  // values to paste into config.local.json are on the page.
  //
  // Public by necessity (you need it BEFORE anything is configured) and safe by
  // construction: every value echoed back came out of a token the caller sent, so
  // a stranger learns nothing they did not already have. Knowing an AUD tag
  // forges nothing — the signature is still checked.
  app.get('/api/auth/access-info', (req, res) => {
    const seen = cfaccess.peek(req);
    const team = require('./config').get('cfAccessTeam', null);
    const aud = require('./config').get('cfAccessAud', null);
    res.json({
      accessInFront: !!seen,
      seen,
      configured: { cfAccessTeam: !!team, cfAccessAud: !!aud },
      matches: seen && !seen.malformed
        ? { iss: !!team && seen.iss === cfaccess.issuer(), aud: !!aud && seen.aud.includes(aud) }
        : null,
      hint: !seen
        ? 'No Cloudflare Access assertion on this request. Either Access is not in '
          + 'front of this hostname, or you reached the origin directly (not through the tunnel).'
        : 'Copy `aud[0]` into cfAccessAud, and the middle of `iss` '
          + '(https://TEAM.cloudflareaccess.com) into cfAccessTeam.',
    });
  });
  ...
  app.get('/api/auth/me', async (req, res) => {
    const uid = auth.verify(auth.readCookie(req.headers.cookie));
    let u = uid ? state.users[uid] : null;
    // Behind Access the browser never sees a sign-in screen at all: the identity
    // is already on the request, so resolve it here and answer with the result.
    if (!u && cfaccess.configured()) {
      try {
        const claims = await cfaccess.identify(req);
        if (claims) {
    ...
    res.json({
      ...
      // Null when Access is in front — the client then knows not to load Google's
      // script at all, rather than showing a button nobody needs to press.
      clientId: cfaccess.configured() ? null : auth.clientId(),
      via: cfaccess.configured() ? 'cloudflare' : 'google',
